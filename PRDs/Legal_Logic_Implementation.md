...
| 14 | 民间借贷：无约定利率、约定年化利率为 0 时，逾期等「算到哪一日」是否与上条同一公式 | **是**，与第 13 条**同一公式**（仍以到期日次日为逾期起点等既有条款为前提，见 §1）。**实现**：`agreed_annual_rate` 为 **0** 与 **未提供** 采用**同一套**无约定计息规则（期内零息、逾期一年期 LPR）；**须填 `due_date`**。 |
...
1. **输入**: 本金 `P`，借款日 `D1`，还款列表（同日可合并），截止日 `D_end`（**含当日**），可选用约定年化、到期日、`filing_date` / `lpr_document_month`。**约定年化利率为 0** 与 **未提供约定利率** 在计息上**等同无约定**（期内零息、逾期一年期 LPR），且 **必须提供 `due_date`**（§0.1 第 14 条）。
...
3. **分段**: 在时间上切开 **分界日**，及（无约定或约定年化=0 时）**到期日次日**逾期起点；逾期且无约定利率的区间再按 **LPR 报价发布日**细分。
...
| 约定利率 0 视同无约定 + 须填到期日（§0.1 第 14 条） | **已实现** | `agreed_annual_rate` 为 None 或 0 时 `due_date` 必填；计息走无约定分支 |
