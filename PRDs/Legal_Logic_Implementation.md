...
**现行代码**仍按下述「欠租区间求交」实现；**业务已定稿**的变更方向见 **§0.1 第 15–16、18 条**（交租日「距月初」等见历史回复，实现迭代时一并改 `RentalRequest` / `engine.py`）。

- **子任务（现行）**：在 **欠租区间与租期（可选）的交集** 内，按 **逐自然月** 生成各期租金的滞纳金；交集为空则报错。
- **子任务（已定稿，待代码对齐）**：滞纳金计至 **`filing_date`（含）**；**欠租统计区间不裁滞纳金**；各月是否出滞纳金按 **§0.1 第 16 条**（与欠租区间脱钩）。
- **应交租日（现行）**：`该月最后日历日 − rent_due_days_before_month_end`（0 表示当月最后一日）；若 N 过大导致落到上月，**钳制为该月 1 日**。
- **应交租日（已定稿，待代码对齐）**：改为 **距当月初的自然日数**（字段与实现待迭代）。
- **起算点（现行）**：应交租日 **次日**；与欠租区间求交：**计息起点 = max(应交租次日, 欠租区间起点)**，**计息止点（不含）= 欠租区间末日 + 1 日**（即末日计入滞纳金）。
- **起算点（已定稿，待代码对齐）**：仍为应交租日 **次日**；止点为 **起诉日次日（半开）**，即含起诉日当日；各月是否纳入见 §0.1 第 16 条。
